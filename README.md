...
| 用户注册 | ✅ PASS | 3个用户成功注册 |
| 创建拼单 | ✅ PASS | 设置最少3人成团 |
| 1人未成团 | ✅ PASS | 状态=FORMING |
| 2人未成团 | ✅ PASS | 状态=FORMING |
| 用户加入拼单 | ✅ PASS | B/C用户成功加入 |
| 3人成功成团 | ✅ PASS | 状态=FORMED |
| 订单自动生成 | ✅ PASS | RocketMQ触发创建 |
| 八折优惠计算 | ✅ PASS | 原始67元→折后53.6元 |
| 费用分摊 | ✅ PASS | 按菜品金额比例分摊优惠 |
| 订单支付 | ✅ PASS | 支付弹窗一键支付 |

### 费用计算验证

```
用户A: 幽兰拿铁 x2 = 36元 → 折后28.80元, 优惠7.20元
用户B: 声声乌龙 x1 = 15元 → 折后12.00元, 优惠3.00元
用户C: 桂花弄 x1 = 16元 → 折后12.80元, 优惠3.20元
─────────────────────────────────────────
原始总计: 67元, 折后: 53.60元, 优惠: 13.40元 (80%) ✅
```

### 商家数据

- **16个商家**：茶颜悦色、麦当劳、海底捞、外婆家、必胜客、肯德基、喜茶、瑞幸咖啡等
- **64个菜品**：每个商家4个代表性菜品
- 支持分类筛选（快餐、奶茶、火锅、中餐、西餐等）
...
6. ✅ 订单金额从settlements计算
7. ✅ 重复订单检查
8. ✅ PoolFormedEvent 空值处理
9. ✅ 订单详情API返回500

### 技术说明

> **数据库说明**：本项目使用 H2 内存数据库 (MySQL兼容模式) 便于开发测试。生产环境可无缝切换到 MySQL 8.0+，只需修改 `application.yml` 中的数据源配置。
